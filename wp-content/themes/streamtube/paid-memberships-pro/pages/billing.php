...
<?php printf(
	'<div class="%s">',
	esc_attr( pmpro_get_element_class( 'pmpro_billing_wrap' ) )
)?>
<?php
		...
		<ul>
			...
			if ( 'individual' === $subscription_gateway_obj->supports( 'payment_method_updates' ) ) {
				...
				<li>
					<?php printf(
						'<strong>%s </strong> (%s)',
						esc_html( $pmpro_billing_level->name ),
						esc_html( $pmpro_billing_subscription->get_cost_text() )
					)?>
				</li>
				<?php
			...
			<form id="pmpro_form" class="<?php echo esc_attr( pmpro_get_element_class( 'pmpro_form' ) ); ?>" action="<?php echo esc_url( pmpro_url( "billing", "", "https") ) ?>" method="post">
				...
				<div class="<?php echo esc_attr( pmpro_get_element_class( 'pmpro_message pmpro_alert', 'pmpro_account_loggedin' ) ); ?>">
					<?php
						...
						echo wp_kses( sprintf( __('You are logged in as <strong>%s</strong>. If you would like to update your billing information for a different account, <a href="%s">log out now</a>.', 'paid-memberships-pro' ), $current_user->user_login, wp_logout_url( home_url(add_query_arg(array(), $wp->request)) ) ), $allowed_html );
					?>
					...
					<div class="<?php echo esc_attr( pmpro_get_element_class( 'pmpro_checkout-fields' ) ); ?>">
						...
						<div class="<?php echo esc_attr( pmpro_get_element_class( 'pmpro_checkout-field pmpro_checkout-field-bemail', 'pmpro_checkout-field-bemail' ) ); ?>">
							...
							<?php printf(
								...
								esc_attr( pmpro_get_element_class( 'input', 'bemail' ) ),
								esc_attr($bemail)
							)?>
						...
						<div class="<?php echo esc_attr( pmpro_get_element_class( 'pmpro_checkout-field pmpro_checkout-field-bconfirmemail', 'pmpro_checkout-field-bconfirmemail' ) ); ?>">
							...
							<?php printf(
								...
								$pmpro_email_field_type ? 'email' : 'text',
								esc_attr( pmpro_get_element_class( 'input', 'bconfirmemail' ) ),
	...
	<?php } elseif ( pmpro_hasMembershipLevel() ) {
		...
		?>
		<p class="text-muted mb-0"><?php esc_html_e("This subscription is not recurring. So you don't need to update your billing information.", 'paid-memberships-pro' );?></p>
		<?php
	} else {
