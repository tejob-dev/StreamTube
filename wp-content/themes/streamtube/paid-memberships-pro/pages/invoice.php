		...
		<ul>
			...
			<?php if ( ! empty( $pmpro_invoice->status ) ) { ?>
				...
				<?php
					...
					printf(
						'<span class="invoice-%s">%s</span>',
						esc_attr( sanitize_html_class( $display_status ) ),
						$display_status
					)
				?>
				</li>
		...
		if($invoices)
		{
			...
			<?php
				foreach($invoices as $invoice)
				{
					?>
					<tr>
						<td data-title="<?php esc_html_e('Date', 'paid-memberships-pro' ); ?>"><a href="<?php echo esc_url( pmpro_url("invoice", "?invoice=" . $invoice->code ) ) ?>"><?php echo esc_html( date_i18n( get_option("date_format"), strtotime( get_date_from_gmt( date( 'Y-m-d H:i:s', $invoice->timestamp ) ) ) ) )?></a></td>
						<td data-title="<?php esc_html_e('Invoice #', 'paid-memberships-pro' ); ?>">
							<a class="badge bg-info text-white" href="<?php echo esc_url( pmpro_url("invoice", "?invoice=" . $invoice->code ) ) ?>"><?php echo esc_html( $invoice->code ); ?></a>
						</td>
						<td data-title="<?php esc_html_e('Level', 'paid-memberships-pro' ); ?>">
							<span class="badge bg-secondary">
								<?php echo esc_html( $invoice->membership_level_name );?>
							</span>
						</td>
						<td data-title="<?php esc_html_e('Total Billed', 'paid-memberships-pro' ); ?>">
